...
이번주 작업
- 싱가포르 고객사를 위한 컴포넌트에 전체 메시지 적용 및 하면서 사용하지 않는 컴포넌트 제거 
	- cm 특화, 과거에 만들어두었지만 사용하지않는 컴포넌트들 제거 
- 인천공항 소스반입 신청해서 이번주 배포예정
- 한국인터넷진흥원 미팅이 있어 미팅준비 및 배포준비 및 배포 
...
- 더미캐시 있는거 확인하기,
- cron으로 해당 도커 빌드 더미 캐시를 날리는 크론제작, 

1.포탈 -> L4접근할때 HTTPS가 필수이다.
2.*HTTP(80)**를 그대로 사용하면 데이터가 암호화되지 않은 '평문'으로 전송되어, 중간에 누군가 패킷
을 가로채면 내용을 그대로 볼 수 있는 **스니핑(Sniffing)** 위험에 노출됩니다.

## 1. L4 ↔ 서버 간 헬스 체크(Health Check) 허용

L4는 서버가 살아있는지 주기적으로 확인합니다. 이 신호가 차단되면 L4는 서버가 죽었다고 판단해 트래픽을 보내지 않습니다.

- **포트(Port):** 서비스 포트(예: 80, 443, 8080 등)를 허용해야 합니다.
    
- **프로토콜:** * **ICMP (Ping):** 장비 자체가 네트워크에 연결되어 있는지 확인할 때 사용합니다.
    
    - **TCP:** 특정 서비스 포트가 열려 있는지 확인할 때 사용합니다.
        
    - **HTTP/HTTPS:** 특정 URL(예: `/health`)에 접속하여 정상 응답(200 OK)이 오는지 확인할 때 사용합니다.