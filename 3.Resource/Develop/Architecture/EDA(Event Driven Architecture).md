...
DDD(Domain-Driven Design, 도메인 주도 설계)는 복잡한 비즈니스 도메인을 중심으로 소프트웨어를 설계하는 방법론입니다.

1) **비즈니스 Domain별로 나누어 설계**하는 방식입니다.

   기존의 어플리케이션 설계가 비즈니스 Domain에 대한 이해가 부족한 상태에서 설계 및 개발되었다는 반성에서 출발하였습니다. DDD에서는 기존의 현업에서 IT로의 일방향 소통구조를 탈피하여 **현업과 IT의 쌍방향 커뮤니케이션**을 매우 중요하게 생각합니다.
...
DDD는 복잡한 도메인 문제를 해결하기 위해 비즈니스 도메인 자체에 집중해서 설계를 진행하는 방법론입니다.

'도메인 모델'을 중심으로 코드와 비즈니스 로직을 일치시키는 개념


DDD는 복잡한 **비즈니스 도메인을 중심**으로 **소프트웨어를 설계하는 방법론**입니다. 저희 프로젝트에서는 DDD의 핵심 요소들을 부분적으로 적용했습니다.

- 도메인 계층과 인프라 계층 완전 분리
- 도메인별 패키지 구조
- 순수 비즈니스 로직 - 도메인 관련
...
| ScheduledExecutorService | 스케줄링 엔진    | 8개 스레드로 동시 처리     |
| ------------------------ | ---------- | ----------------- |
| CompletableFuture        | 비동기 결과 관리  | 즉시 반환, 나중에 완료     |
| AtomicInteger            | 스레드 안전 카운터 | 타임아웃, 예외 카운트      |
| ConcurrentHashMap        | 작업 관리      | 실행 중인 작업 추적       |
| Runnable                 | 폴링 로직      | 실제 API 호출 및 상태 확인 |
|                          |            |                   |

"주기적인 요청을 위해 다음과 같은 클래스들을 사용했습니다:

1. ScheduledExecutorService: 8개 스레드로 동시 폴링 처리
2. CompletableFuture: 비동기 결과 전달로 사용자 경험 개선
3. AtomicInteger: 스레드 안전한 타임아웃 및 예외 카운트
4. ConcurrentHashMap: 실행 중인 작업의 생명주기 관리
5. Runnable: 실제 폴링 로직 구현

이를 통해 20-30개 동시 배포를 안정적으로 모니터링하면서
사용자는 즉시 응답을 받고 나중에 결과를 알 수 있습니다."