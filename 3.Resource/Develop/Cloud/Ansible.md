...
2.Ansible
- 장점 : 보통의 설치 도구들은 해당 노드에 에이전트가 설치되어야하지만, 이건 에이전트리스이다. 
- **동작 방식:** Packer가 vSphere에 VM을 만들고 네트워크를 잡으면, 사용자 맥북에 있는 Ansible이 **SSH**를 타고 해당 VM에 접속해서 플레이북을 실행합니다.

3.Helm - Pod 패키지 매니저
- helm install my-portal ./portal-chart 등의 명령어로 간단하게 실행이 가능하도록 설정
- Prometheus + Grafana 


### 3. 추천하는 구성 (The Best Practice)

가장 권장하는 방식은 **`kube-prometheus-stack`**이라는 Helm 차트를 사용하는 것입니다.

- **장점:** 프로메테우스, 그라파나, 알람매니저뿐만 아니라 위에서 말한 **kube-state-metrics**와 **기본 대시보드**까지 한 방에 설치됩니다.
    
- **대시보드 활용:** 설치 후 그라파나에 접속하면 이미 만들어진 **"Kubernetes / Compute Resources / Pod"** 대시보드가 있습니다. 이걸 클릭하면 특정 Pod의 상태를 바로 모니터링할 수 있습니다.


---

### 1. 세트 메뉴(`kube-prometheus-stack`)의 구성품

이 패키지를 설치하면 다음과 같은 도구들이 한 번에 들어옵니다.

- **Prometheus:** 데이터 수집 및 저장 (엔진)
- **Grafana:** 수집된 데이터 시각화 (대시보드)
- **Alertmanager:** 장애 발생 시 알림 발송 (비상벨)
- **kube-state-metrics:** **Pod의 상태**(Pending, Running 등) 데이터를 생성해주는 도구
- **node-exporter:** 각 노드(서버)의 하드웨어 상태 수집기
- **Prometheus Operator:** 이 모든 복잡한 설정을 쿠버네티스 안에서 자동으로 관리해주는 관리자

---

구현 순서 

## CMP 솔루션 배포 전체 로드맵 (The Master Sequence)

가장 권장되는 순서는 **[인프라 → 공통 서비스 → 레지스트리 → 데이터/이미지 → 앱]** 순입니다.
Ansible에서 이 모든 과정을 한 번에 하는 것이 가능하다.

### 1단계: OS 및 K3s 엔진 구성 (Ansible)

- **작업 내용:** 방화벽/SELinux 설정, 커널 최적화, K3s 마스터/워커 클러스터 구성.
- **이유:** 운동장(K8s)이 먼저 만들어져야 그 위에 뭘 깔 수 있습니다.

### 2단계: 스토리지(Longhorn) 및 네임스페이스 (Ansible/Helm)

- **작업 내용:** Longhorn 설치, 필요한 **Namespace**(`monitoring`, `harbor`, `portal` 등) 미리 생성.
- **이유:** DB나 모니터링 툴은 데이터를 저장할 **"공유 디스크(Longhorn)"**가 있어야만 정상적으로 뜰 수 있습니다.

### 3단계: 모니터링(kube-prometheus-stack) 설치 (Helm)

- **작업 내용:** 프로메테우스와 그라파나 설치.
- **이유:** 솔루션이 배포되는 과정을 모니터링하기 위해 미리 띄워두는 것이 좋습니다.


### 4단계: 레지스트리(Harbor) 설치 (Helm)

- **작업 내용:** Harbor 설치 및 인증서 설정.
- **이유:** 우리 솔루션의 이미지(Portal, DB 등)를 보관할 **창고**가 먼저 있어야 합니다.
    
### 5단계: 이미지 업로드 및 외부 연동 (Script/Manual)

- **작업 내용:** 로컬에 있는 `.tar` 이미지들을 Harbor로 `push`.
- **이유:** 이제 창고(Harbor)가 생겼으니 물건(이미지)을 채워 넣어야 앱을 띄울 수 있습니다.
    

### 6단계: 어플리케이션 스택 배포 (Helm)

- **작업 내용:** Redis → DB → Portal 순서로 배포.
- **이유:** 포털은 DB에 의존하기 때문에 DB가 먼저 안정적으로 뜨는 것을 확인하고 포털을 올려야 합니다.

---


• 빠르게 개발하고 배포하는 조직에서 주도적으로 일해 보신 분이 필요합니다.
• 올웨이즈 Growth Feature/Commerce Platform 제품의 웹/앱 컴포넌트 설계, 개발, 테스트 및 운영을 담당합니다.  
• 서비스 아키텍처 설계부터 출시까지 전 과정에 참여합니다.  
