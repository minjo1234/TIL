...
1.KEYS → `SCAN` 리팩토링 (성능 측정)

- **측정 포인트:** 데이터 10만 개 이상을 넣고 `getAllKeys()`를 실행했을 때, **다른 API들의 응답 속도(Latency)**가 튀는지 확인해 보세요. `KEYS`는 다른 요청을 멈추게 하지만, `SCAN`은 다른 요청의 속도를 일정하게 유지해 줍니다.
- **이력서 강조:** "싱글 스레드 기반 Redis의 Blocking 문제를 방지하기 위해 `SCAN`을 도입하여, 대량의 키 조회 시에도 타 서비스 API의 가용성을 100% 보장함."

2.캐시 쉐도 (Cache Stampede) 방지

- 사용자님 코드에 `CacheType`별로 TTL이 다르게 설정되어 있다면 이미 기초적인 대응은 된 것입니다.
- **추가 리팩토링:** 만약 아주 중요한 데이터라면, TTL이 만료되기 직전에 누군가 한 명만 DB에 가서 업데이트해오는 **'확률적 만료(Probabilistic Early Expiration)'**나 **'분산 락(Redisson)'**을 공부해서 적용해 보시는 걸 추천합니다.
- 분산 락  정의해보기.
...
### 1. 인프라 이중화 (Sentinel / Cluster)

- **전략:** 레디스 서버가 죽어도 다른 레디스가 즉시 그 역할을 이어받게 함.
    
- **장점:** 애플리케이션(자바 코드) 입장에서는 레디스가 잠깐 멈춘 것 같지만 금방 다시 정상 작동함. **데이터 유실이나 DB 부하 폭발을 원천 차단**할 수 있음.
    
- **비용:** 최소 3대 이상의 서버가 필요하므로 인프라 비용이 증가함.
    

### 2. 애플리케이션 Fallback (DB 직접 조회)

- **전략:** 레디스 응답이 없으면 "에러"를 뱉는 게 아니라, "DB로 우회"해서 데이터를 가져옴.
    
- **장점:** 레디스 서버가 1대뿐이어도 서비스가 죽지 않고 유지됨.
    
- **단점:** 레디스가 죽어있는 동안 모든 트래픽이 DB로 몰리기 때문에, **DB가 버티지 못하고 같이 죽을 위험(Cascading Failure)**이 있음.


---

## Redis 도입 및 최적화 요약 (Portfolio/Resume 버전)

### 1. 문제 상황 (Background)

- **기존 환경:** 단일 서버 기반의 CMP 솔루션으로, `Local Cache`와 멤버 변수를 통해 토큰, 권한(Role), 메뉴, 세션 데이터를 관리함.
- **한계점:** 서비스 확장(Scale-out) 시 서버 간 데이터 불일치(Inconsistency)가 발생하여 다중 서버 환경에서의 데이터 동기화 문제에 직면함.

### 2. 해결 방안 (Implementation)

분산 환경에서의 데이터 일관성 보장을 위해 **Redis**를 전역 캐시 및 저장소로 도입하고, 데이터 특성에 따라 이원화된 구현 전략을 수립함.

- **Spring Cache Abstraction 활용:** 조회 빈도가 높고 상태 변화가 거의 없는 정적 데이터(메뉴 등)에 적용하여 생산성 향상.
- **RedisTemplate 기반 Repository 패턴 구축:** 비즈니스 로직 내 복잡한 데이터 가공 및 실시간 수정이 필요한 데이터(세션, 권한 등)를 위해 추상 클래스(`RedisBaseRepository`)를 활용한 맞춤형 자료구조(Hash, List 등) 제어.

### 3. 최적화 및 안정성 확보 (Optimization & Resilience)

대용량 트래픽과 장애 상황을 고려하여 시스템의 가용성을 극대화함.

- **성능 최적화 (`KEYS` → `SCAN`):** Redis의 Single Thread 특성으로 인한 Blocking 문제를 방지하기 위해, 전체 키 조회 로직을 `SCAN` 방식으로 리팩토링하여 대량 트래픽 상황에서도 서비스 응답성을 유지함.
- **장애 대응 전략 (Failover):** 단일 컨테이너 환경의 Redis 장애(SPOF)를 대비하여, Redis 연결 실패 시 데이터베이스(RDB)를 직접 조회하도록 **Fallback 로직 및 예외 처리**를 설계하여 서비스 연속성을 보장함.

---

- **'전략 패턴' vs '템플릿 메소드 패턴':** 보여주신 `RedisBaseRepository` 코드는 부모가 골격을 만들고 자식이 구체적인 값을 채우는 방식이라 **'템플릿 메소드 패턴'**이 더 정확한 용어입니다. 이 점을 정확히 기술하면 설계 역량을 더 높게 평가받을 수 있습니다.
    
- **수치적 성과 (있다면):** 만약 가능하다면, "로컬 캐시에서 레디스로 전환 후 서버 간 데이터 정합성 오류 100% 해결"이라거나, "응답 속도 O% 개선" 같은 수치를 한 줄 덧붙이면 금상첨화입니다.
    
- **다음 스텝 언급:** "향후 Redis Sentinel을 도입하여 인프라 레벨의 고가용성(HA)을 구축할 계획입니다"라는 문구를 넣으면, 현재의 한계(단일 컨테이너)를 알고 있고 이를 해결할 로드맵도 있는 개발자로 보입니다.

---
### 1. RedisBaseRepository = 템플릿 메서드 패턴 (Template Method)

"공통 로직의 재사용"에 집중한 패턴입니다.

- 구현 방식: 상속(Inheritance)
- **핵심 로직:** `key()`, `delete()`, `exists()` 등 레디스를 다루는 **실행 흐름(Step)**은 부모가 미리 다 짜놓았습니다.
- **확장 포인트:** `getBaseKey()`, `getDefaultTtl()` 처럼 자식마다 달라져야 하는 **세부 데이터**만 추상 메서드로 비워두고 자식이 채우게 합니다.
- **결론:** "레디스라는 도구를 사용하는 **방법론(틀)**을 규격화했다"고 볼 수 있습니다.

---
### 2. Local vs Redis 구현체 분리 = 전략 패턴 (Strategy)

"상황에 따른 알고리즘의 교체"에 집중한 패턴입니다.

- **구현 방식:** **인터페이스(Interface)와 합성(Composition)**
- **핵심 로직:** 예를 들어 `CacheRepository`라는 인터페이스가 있고, 이를 구현한 `LocalCacheRepository`와 `RedisCacheRepository`가 있는 구조입니다.
- **확장 포인트:** 서비스(Service) 입장에서는 "저장하고 조회한다"는 인터페이스만 알면 됩니다. 내부적으로 이게 **내 메모리(Local)**에 저장되는지, **외부 서버(Redis)**에 저장되는지는 외부 설정(주입)에 의해 결정됩니다.
- **결론:** "환경(단일 서버 vs 분산 서버)에 따라 **저장 전략**을 통째로 갈아 끼울 수 있게 만들었다"고 볼 수 있습니다.

---

전략 패턴 : 인터페이스를 구현하고 실행할 때 실제 구현체를 주입받아 사용합니다.
템플릿 메서드 패턴 : 부모가 추상(abstract) 메서드를 비워둡니다.