...
- 실무에서 자연스럽게 로직을 공통 로직을 구현하는중 전략 패턴과 템플릿 메서드 패턴을 언제 어느 상황에 사용해야 하는지 어떠한 차이점이 있는지 정확히 이해를 못 하고 있다는 생각이 들었다. 둘다 공통 부모를 하지고 하위 클래스에서 구현체를 구현하지만 어떤 차이가 있는것일지 분석하기로 한다.
