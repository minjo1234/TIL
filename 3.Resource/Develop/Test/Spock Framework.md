...
- mock (stubbing vs mocking) - 메서드에서 수신하는 호출을 등록해서 실제 호출이 잘 되었는지(호출여부, 호출 횟수, 호출 인자 - 


- stub (stubbing ) - 호출 결과를 고정된 값으로 미리 정의, 실제 호출 여부나 횟수를 신경 쓰지 않는다. (고정된 리턴값 제공, 상태(state) 메서드를 어떤 값을 반환해야 하는가 ?, 미리 정의해둔 값을 반환, 호출 여부 따지징)
