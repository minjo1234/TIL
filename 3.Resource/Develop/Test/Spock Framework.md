...
- mock (stubbing vs mocking) - 메서드에서 수신하는 호출을 등록해서 실제 호출이 잘 되었는지(호출여부, 호출 횟수, 호출 인자) , (메서드 호출 여부/횟수 검증, 행위(behavior) 이 메서드가 실제로 호출되었는가? 몇? 번? 어떤 인자로?, 필요시 정의가능, 주 목적은 아님,)
