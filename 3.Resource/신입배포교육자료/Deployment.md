...
db_container에서는 에러가 발생하지 않음 
web_container에서는 에러가 발생함 : 
- 이건 **애플리케이션 레벨에서 중복 체크를 하다가 에러를 로깅**하는 것일 수 있습니다.

근데 이건 warning이라서 확인해볼것은 분산스위치이다. 분산스위치의 경우 수집이 안되는것으로 보이는데 


- dvportgroup에 속하는 network 일 때 사용률이 수집이 되는경우가 존재한다.


- PerfQuerySpecBuilder.java
- VSphereMetricsImpl.java
